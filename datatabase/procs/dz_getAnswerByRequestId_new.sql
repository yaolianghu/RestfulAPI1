...
DROP PROCEDURE IF EXISTS dz_getAnswersByRequestId_new$$

CREATE PROCEDURE dz_getAnswersByRequestId_new(in quoteRequestId int)
BEGIN 
	select distinct paramKey, questionId, a.type, question, answer
	from (
	select q.question_id as paramKey, qrp.param_key as questionId, q.type,q.supplier_text as question, q.display_order,
	...
	select distinct q.question_id as paramKey,  qrp.param_key as questionId, q.type,
	case when q.type = 'textmultiple' and qr.category_id = 10125 then a.label
         else a.supplier_text
    end as question, q.display_order, qrp.param_value as answer
    from quote_request_param qrp 
	join quote_request qr on qr.quote_request_id = qrp.quote_request_id
